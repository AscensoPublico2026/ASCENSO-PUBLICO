...
// ── Ficha de datos visible mientras el curso está en espera ──────────────────
function DatosCurso({ curso, convNombre, cargoNombre }: {
  curso: any; convNombre: string; cargoNombre: string;
}) {
  type Fila = { icon: string; label: string; value: string };
  const filas: Fila[] = [];

  if (curso.numero_inscripcion)
    filas.push({ icon: "🏷️", label: "N.° de inscripción", value: curso.numero_inscripcion });
  if (curso.opec)
    filas.push({ icon: "🔢", label: "N.° OPEC", value: curso.opec });
  if (cargoNombre)
    filas.push({ icon: "💼", label: "Cargo", value: cargoNombre });
  if (convNombre)
    filas.push({ icon: "📋", label: "Convocatoria", value: convNombre });
  if (curso.nivel)
    filas.push({ icon: "🎯", label: "Nivel", value: curso.nivel.charAt(0).toUpperCase() + curso.nivel.slice(1) });
  if (curso.fecha_vencimiento)
    filas.push({
      icon: "📅", label: "Acceso válido hasta",
      value: new Date(curso.fecha_vencimiento).toLocaleDateString("es-CO", {
  ...
  return (
    <div style={{ marginTop: 18, background: "rgba(255,255,255,.7)", border: "1px solid #F0DCB0", borderRadius: 12, overflow: "hidden" }}>
