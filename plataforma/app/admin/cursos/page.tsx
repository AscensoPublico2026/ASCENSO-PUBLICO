...
export default async function AdminCursos() {
  ...
  const { data: cursos } = await supabase
    .from("cursos")
    .select("id,cargo_nombre,nivel,estado,fecha_compra, profiles(nombre,correo)")
      ...
      {(!cursos || cursos.length === 0) ? (
        ...
        <div style={{ overflowX: "auto", background: "#fff", borderRadius: 14, border: "1px solid var(--gris-borde)" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr><th style={th}>Cliente</th><th style={th}>Cargo</th><th style={th}>Nivel</th><th style={th}>Estado</th><th style={th}></th></tr></thead>
            <tbody>
              {cursos.map((c: any) => (
                <tr key={c.id}>
