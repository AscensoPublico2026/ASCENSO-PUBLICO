...
**Cupos administrables sin identificación adicional:**
...
- ✅ **Flujo de compra restaurado:** se retiró el campo de identificación y toda dependencia de cifrado del formulario, procesamiento de pagos y panel administrativo.
- ✅ **Sin secreto adicional:** la compra ya no depende de una clave nueva. Si la migración anterior alcanzó a crear columnas o una tabla de identificación en Supabase, pueden permanecer vacías; el sistema ya no las consulta ni escribe.
- ⚠️ **Configuración de cupos:** si aún no existe `configuracion_cupos`, ejecutar una vez `plataforma/supabase/migracion-configuracion-cupos.sql`.
...
- **Tablas Supabase:** `profiles`, `convocatorias`, `preregistros`, `configuracion_cupos`, `cursos`, `guias_curso`, `pagos` y `analytics_events`. Enum `tipo_guia`: `general | nivel | funcional | bonus | simulacro`.
...
### 9.2 "Conoce tu Entidad" (ENT-*) — MODELO PREMIUM DEFINIDO ✅ (actualizado ago-2026)
**Nuevo benchmark aprobado por el cliente: `ENT-DIAN-01`** (reemplaza a ENT-IDV-01 como referencia). Toda guía ENT debe ser **GENERAL y REUTILIZABLE por entidad** (NO menciona cargo/código/OPEC en el cuerpo → una sola ENT sirve para todos los cursos futuros de esa entidad) y **PREMIUM, al nivel de una guía de Desarrollo** (~4.000–4.500 palabras, ~15 secciones navegables). Secciones del modelo: Bienvenida · Identidad · Misión/Visión/Valores · Historia (línea de tiempo) · Qué hace · Lo que administra (tabla) · Conceptos clave (10-12 definiciones) · Herramientas (acordeones) · Organización (+ diagrama) · En cifras (datos reales) · Servicios y canales · Transparencia · Marco legal · Para tu prueba · Fuentes oficiales (15-30 enlaces). Con muchos enlaces oficiales verificados, frases clave en `<mark>` (subrayado dorado), datos reales (nunca inventar), marca navy/oro, barra de progreso y nav activo. Estándar documentado en `.kiro/steering/estandar-conoce-tu-entidad.md`. La versión vieja (ENT-IDV-01, ENT-HUH-01) es corta y debe elevarse a este modelo cuando se reutilice.
